...
async function importLiveProviders() {
  try {
    ...
    if (process.env.DATA_MODE !== 'live') {
      throw new Error('Must be in live mode to import live providers');
    }

    // Clear existing data in live mode
    await prisma.provider.deleteMany();

    // Read and parse CSV
    const csvPath = path.join(process.cwd(), 'data', 'live_mode', 'providers.csv');
    ...
    console.log('Successfully imported live providers');
  } catch (error) {
    console.error('Error importing live providers:', error);
    throw error;
  } finally {
